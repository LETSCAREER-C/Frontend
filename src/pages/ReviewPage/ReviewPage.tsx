...
const ReviewPage: React.FC = () => {
  ...
  return (
    <div className="justify-center bg-Neutral-grayscale-90 min-h-screen flex flex-col items-center">
      <img className="flex w-full h-[410px] mt-[60px] mb-[80px] bg-Neutral-grayscale-75" />
      {/* <div className="flex h-[24px] pt-[20px]">
           모집 마감까지 남은 날짜 => 백엔드에서 deadline으로 보내줌 / 리쿠르트 태그 / 커리어 필터 /타입 태그 가져오기 
           여기에 deadline 받아오기 
          <RecruitTags status={RecruitState.RECRUITING} />
          <CareerTags status={"career-explore"} />
          <TypeTags status={"LIVECLASS"} />
        </div> */}
      ...
      <button className="h-[42px] w-[42px] sm:h-[52px] sm:w-[52px]" onClick={handleBackClick}>
        <BackBtn />
      </button>
